...
- **Secure Authentication**: Password hashing using **jBCrypt** with advanced validation.
- **Role-Based Dashboards**: Specialized views and permissions for Students, Tutors, and Administrators.
- **Two-Factor Authentication (2FA)**: Enhanced desktop security to protect user data.
- **Advanced Profiles**: Dynamic profile picture fetching (Gravatar integration), custom local uploads, and real-time XP/status tracking.
- **OAuth Integration**: Secure external login capabilities via Google and LinkedIn.
